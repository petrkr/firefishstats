"""
Creditas Bank API implementation
"""
import requests
import json
import logging
...
class CreditasClient(GenericBankAPI):
    """Creditas Bank API client"""

    API_BASE_URL = "https://api.creditas.cz/oam/v1"

    def __init__(self, api_token: str, account_id: str, **kwargs):
        """
        Initialize Creditas API client

        Args:
            api_token: Creditas API authentication token
            account_id: Creditas account ID
        """
        super().__init__(api_token, account_id, **kwargs)
    ...
    def fetch_transactions(self, page_size: int = 50, page_index: int = 0) -> Optional[Dict]:
        ...
        try:
            url = f"{self.API_BASE_URL}/account/transaction/search"
            payload = {
